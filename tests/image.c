...
int main(void) {
    ...
    bool vert_src_ok = sys_read_entire_file(get_temp(), STR("tests/shaders/image.vert.osl"), &vert_src);
    assert(vert_src_ok);
    ...
    sys_write_entire_file(STR("test.spv"), (string){frag_code_size, (u8*)frag_code});
    ...
    // by copying through the image copy target view.
    
    Oga_Image_Copy_Target_View *copy_dst = 0;
    Oga_Image_Copy_Target_View_Desc copy_dst_desc = (Oga_Image_Copy_Target_View_Desc){0};
    ...
    copy_dst_desc.flags = OGA_IMAGE_COPY_DST;
    if (!check_oga_result(oga_init_image_copy_target_view(context, copy_dst_desc, &copy_dst)))
        return 1;
    ...
    Oga_Image_Copy_Desc image_dst_desc = (Oga_Image_Copy_Desc){0};
    ...
    oga_uninit_image_copy_target_view(copy_dst);
    ...
    oga_deallocate_memory(image_ptr);
