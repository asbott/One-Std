...
unit_local void put_ir_header(IR_Program *prog) {
	...
	for (u64 i = 0; i < decl_count; i += 1) {
		...
		switch (decl->type) {
			...
			case TYPE_FLOAT32:
				if (decl->storage_flags & STORAGE_INPUT_SCALAR) {
					put_ir_instr_2(&prog->init_block, i_broadcast32_scalar, op_reg(prog->base_vgpr++), op_builtin(o_in_buffer, offset));
					
					offset += 4;
				}
				if (decl->storage_flags & STORAGE_INPUT) {
					put_ir_instr_2(&prog->init_block, i_movu, op_reg(prog->base_vgpr++), op_builtin(o_in_buffer, offset));
					offset += 32;
				}
				break;
...
unit_local void put_ir_expr(IR_Block *block, struct Code_Expr *expr)
{
	...
	case EXPR_LITERAL_INT:
		expr->read_reg = 8196+expr->imm_index;
		break;
	case EXPR_TYPE:
		break;
...
unit_local void put_ir_footer(IR_Program *prog) {
	...
	for (u64 i = 0; i < decl_count; i += 1) {
		...
		switch (decl->type) {
			...
			case TYPE_FLOAT32:
				if (decl->storage_flags & STORAGE_OUTPUT) {
					put_ir_instr_2(&prog->exit_block, i_mov, op_builtin(o_out_buffer, offset), op_reg(r+2));
					r += 1;
					offset += 32;
				}
				break;
...
unit_local string text_x64_format_op(Operand op, s64 i) {
	string sop = STR("");
	switch (op.kind) {
		case o_reg:
			sop = (op.reg < 8196 ? tprint("ymm%u", op.reg) : tprint("i%u", i));
			break;
		case o_mem:
			sop = tprint("[ymm%u]", op.reg);
			break;
		case o_builtin:
			switch(op.builtin) {
...
unit_local void emit_text_x64_block(Text_X64_Emitter *x64, IR_Block *block) {
	...
	for (u64 i = 0; i < block->instruction_count; i += 1) {
		...
		s64 i1 = op1.reg-8196;
		s64 i2 = op2.reg-8196;
		string sop0 = text_x64_format_op(op0, i0);
		string sop1 = text_x64_format_op(op1, i1);
		string sop2 = text_x64_format_op(op2, i2);
	...
	for (u64 i = 0; i < statement_token_count; i += 1) {
		...
		if (do_parse_expr) {
			...
			if (next->kind != ';') {
				...
				log(0, "Expected ; after expression, got %s\n", next_str);
				return 1;
			}
		}
...
unit_local void emit_instr_1(Instr instr, Operand op0) {
	string istr = instr_avx_str(instr);
	s64 i0 = op0.reg-8196;
	string sop0 = op0.kind == o_reg ? (op0.reg < 8196 ? tprint("ymm%u", op0.reg) : tprint("i%u", i0)) : tprint("[%s]", op0.addr);
	fprint(out_file, "%s %s\n", istr, sop0);
}
unit_local void emit_instr_2(Instr instr, Operand op0, Operand op1) {
	...
	string sop0 = op0.kind == o_reg ? (op0.reg < 8196 ? tprint("ymm%u", op0.reg) : tprint("i%u", i0)) : tprint("[%s]", op0.addr);
	s64 i1 = op1.reg-8196;
	string sop1 = op1.kind == o_reg ? (op1.reg < 8196 ? tprint("ymm%u", op1.reg) : tprint("i%u", i1)) : tprint("[%s]", op1.addr);
...
unit_local void emit_instr_3(Instr instr, Operand op0, Operand op1, Operand op2) {
	...
	string sop1 = op1.kind == o_reg ? (op1.reg < 8196 ? tprint("ymm%u", op1.reg) : tprint("i%u", i1)) : tprint("[%s]", op1.addr);
	s64 i2 = op2.reg-8196;
	string sop2 = op2.kind == o_reg ? (op2.reg < 8196 ? tprint("ymm%u", op2.reg) : tprint("i%u", i2)) : tprint("[%s]", op2.addr);
