...
int main(int argc, char **argv) {
	...
	*main_block = (IR_Block){0};
    ...
    Easy_Command_Result assembler_res = sys_run_command_easy(tprint("\"C:\\Program Files\\Microsoft Visual Studio\\2022\\Community\\VC\\Tools\\MSVC\\14.42.34433\\bin\\Hostx86\\x64\\ml64.exe\" /c /Fo\"%s.obj\" \"%s.asm\"", in_path, in_path), sys_get_stdout(), sys_get_stdout(), STR(""), true);
    assert(assembler_res.process_start_success && assembler_res.exit_code == 0 || assembler_res.exit_code == 1104);
    
    Easy_Command_Result linker_res = sys_run_command_easy(tprint("\"C:\\Program Files\\Microsoft Visual Studio\\2022\\Community\\VC\\Tools\\MSVC\\14.42.34433\\bin\\Hostx64\\x64\\link.exe\" /DLL /EXPORT:out_buffer /EXPORT:in_buffer /SUBSYSTEM:WINDOWS /ENTRY:DllMain /OUT:%s %s.obj", out_path, in_path), sys_get_stdout(), sys_get_stdout(), STR(""), true);
